...
- What is the probability a randomly selected patient received the placebo
  ...
  0/84 -- not possible. disjoined possibility 
...
3/8 chance for blue, 1/8 for white

We can simulate this "drawing" with the `sample()` function.

```{r sample}
#set.seed(100) -- this makes it the same every time
...
times. -- this is a random simulation
...
```
size argument makes it done as many times as you want, then you have to replace it or else you would only be able to do it 8 times. 

```{r summarize_draws}
counts <- table(draw_results)
prop.table(counts)
```
this reflects the law of large numbers, so the more times you do it the more times you are going to get closer to the actual percentages of the number of marbles. 
...
it would be more likely for red and blue but not very likely for white. it would be more likely for the more common colors. 
...
two_draw_results
```
we did two draws and we created a vairable if draw one is equal to draw two.
...
```
we are doing this the number of times it occurs over the total number of times there was a draw. so is false about 60% of the time and true about 40% of the time.
...
roll_results
```
if one of these is true the other isnt --
...
```
it looks like event 2 is less likely. 
